...
def get_llm_dual_summary(signals, api_key):
    ...
    prompt = f"""
You are a market technical analyst and educator.

Given the following aggregated signals for the market index, write two summaries:
...
def analyze(ticker, company_name=None, horizon="7 Days", lookback_days=None, api_key=None):
    # For now, market is "proxied" by stock agent output (can replace with true market logic)
